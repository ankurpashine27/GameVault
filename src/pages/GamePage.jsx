...
export default function GamePage({ favourites, onToggleFavourite }) {
  const { id } = useParams()
  ...
  if (!game) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Link to="/" className="mt-4 px-6 py-2.5 rounded-lg bg-accent-blue hover:bg-blue-500 text-white font-heading font-semibold transition-colors duration-200">
          ← Back to Catalog
        </Link>
      </div>
    )
    ...
    <>
      <main className="pb-20">
        ...
        <section className={`relative h-[55vh] min-h-[380px] bg-gradient-to-br ${game.color} pt-16 overflow-hidden`}>
          ...
          <Link to="/"
            className="absolute top-20 left-5 sm:left-8 z-10 flex items-center gap-1.5
              text-text-secondary hover:text-text-primary transition-colors text-sm font-medium">
            ← Back to Catalog
          </Link>
          ...
          <div className="relative z-10 h-full flex items-end pb-10 px-6 sm:px-10 max-w-screen-xl mx-auto">
            <div className="max-w-2xl">
              ...
              <p className="text-text-secondary text-base sm:text-lg max-w-lg">{game.description}</p>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-4xl mx-auto px-5 sm:px-8 py-10">
          ...
          {/* Long description */}
          <p className="text-text-secondary text-base leading-relaxed mb-10">{game.longDescription}</p>
            ...
            {game.tags.map(tag => (
              <span key={tag}
                className="text-sm px-4 py-1.5 rounded-full
                  bg-vault-elevated border border-vault-border text-text-secondary capitalize
